...
read_inputfile_enable="0" # читать ли r(t) из файла
...
if [[ "read_inputfile_enable"  -ne "1" ]]; then 
    ...
    #		   summa_fr=$(  bc -l <<< "scale=2; $summa_fr +3*s((2*3.14)*$tt*(5/60)/(15/60))  "       ); # сдвиг фазы +0.0м| амплитуда   7%
    ...
    # низкие частоты
    		   summa_fr=$(  bc -l <<< "scale=11; $summa_fr +0.5*s((2*3.14)*$tt*(5/60)/1)  "      );  # сдвиг фазы +18c| амплитуда 26%
    		   summa_fr=$(  bc -l <<< "scale=11; $summa_fr +0.5*s((2*3.14)*$tt*(5/60)/2)  "      );  # сдвиг фазы +22c| амплитуда 32%
    #		   summa_fr=$(  bc -l <<< "scale=11; $summa_fr +1*s((2*3.14)*$tt*(5/60)/2.6)  "    );  # сдвиг фазы +27c| амплитуда 40%
    #		   summa_fr=$(  bc -l <<< "scale=11; $summa_fr +10*s((2*3.14)*$tt*(5/60)/4)  "      );  # сдвиг фазы +30с| амплитуда 64%
    #		   summa_fr=$(  bc -l <<< "scale=11; $summa_fr +2*s((2*3.14)*$tt*(5/60)/5)  "      );  # сдвиг фазы +41с| амплитуда 65%
    ...
    #		   summa_fr=$(  bc -l <<< "scale=11; $summa_fr +2*s((2*3.14)*($tt+0)*(5/60)/40) "  );  # сдвиг фазы +1 мин| амплитуда 99%
    		   summa_fr=$(  bc -l <<< "scale=11; $summa_fr +1*s((2*3.14)*$tt*(5/60)/100)"      );  # сдвиг фазы +1 мин| амплитуда 99%
    		   summa_fr=$(  bc -l <<< "scale=11; $summa_fr +1*s((2*3.14)*$tt*(5/60)/120)"      );  # 

    #		    [[ $(bc <<< "$tt<(4*60/$sample)") -eq 1 ]] &&  summa_fr=0 
