...
# просто мы суммируем число точек где r != 0 и делим на общее число точек
# тут (!) важно заметить что если эта полоска на 100% загружееа это не значит 
# что cpu на компе загружен на 100%. это НЕТАК. эта полоска просто показывает
# что в кажждый семпл времени цпу хоть чем то но занят. тоесть в каждоый 
# момент времени у нас есть хотя бы один процесс. а нето что у нас много ядерный
# cpu загружен. нет. это несколько друая характиерстика
#
